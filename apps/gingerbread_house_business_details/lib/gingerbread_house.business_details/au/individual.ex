defmodule GingerbreadHouse.BusinessDetails.AU.Individual do
    @moduledoc """
      A struct representing an Australian business of individual structure (e.g. sole
    ...
    defimpl GingerbreadHouse.BusinessDetails.Mapper, for: Individual do
        def to_map(info) do
            %{ type: :individual, country: "AU" }
            |> set_name(info)
            |> set_contact(info)
            |> set_address(info)
            |> set_abn(info)
